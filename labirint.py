...
class Player(GameSprite):
    ...
    def fire(self):
        ...
        if self.x_speed > 0:
            ...
            acorn.x_speed = 6
            acorn.update()
        '''if sprite.spritecollide(dyno,acorns,False):
            boar.x_speed = 20
        elif sprite.spritecollide(dyno2,acorns,False):
            boar.x_speed = 20'''

#ЗМІННІ
s = mixer.Sound('oof1.wav')
s.set_volume(0.5)
mixer.music.load('bad.mp3')
mixer.music.set_volume(0.05)
img =  transform.scale(image.load('field.jpg'),(700,500))
win = transform.scale(image.load('won.jpg'),(700,500))
...
while run:
    ...
    if finish != True:                  
        ...
        final.draw_pic()
        acorns.draw(window)
        if sprite.groupcollide(dynos,acorns,True,True):
            s.play()
        dynos.update()
        dynos.draw(window)
        boar.update()  
